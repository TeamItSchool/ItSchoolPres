...
;(function( window ) {
  ...
  Memory.prototype._setupGame = function() {
    ...
    this.gameMetaHTML = '<div class="mg__meta--left">\
      ...
      <button id="mg__button--restart" class="mg__button">Recommencer</button>\
      </div>';
    this.gameMeta.innerHTML = this.gameMetaHTML;
    this.game.appendChild(this.gameMeta);

    this.gameStartScreenHTML = '<h2 class="mg__start-screen--heading">Bienvenue au Memory Game !</h2>\
      <p class="mg__start-screen--text">Aides Bobine à trouver les paires qui correspondent avec le moins de coups possible !</p>\
      <h3 class="mg__start-screen--sub-heading">Choisis un niveau</h3>\
      <ul class="mg__start-screen--level-select">\
      <li><span data-level="1">Niveau 1 - Lune (4 x 2)</span></li>\
      <li><span data-level="2">Niveau 2 - Soleil (6 x 3)</span></li>\
      <li><span data-level="3">Niveau 3 - Étoile (8 x 4)</span></li>\
      </ul>';
  ...
  /**
   * Memory _startScreenEventsHandler
   *
  ...
  Memory.prototype._winGame = function() {
    ...
    if (this.options.onGameEnd() === false) {
      this._clearGame();
      this.gameMessages.innerHTML = '<h2 class="mg__onend--heading">WaoUh !</h2>\
        <p class="mg__onend--message">Tu as gagné en ' + this.numMoves + ' coups. Bobine te remercie !</p>\
        <button id="mg__onend--restart" class="mg__button">Veux-tu rejouer ?</button>';
